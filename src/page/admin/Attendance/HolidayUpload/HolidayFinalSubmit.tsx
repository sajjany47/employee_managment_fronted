...
const HolidayFinalSubmit = (props: any) => {
  ...
  useEffect(() => {
    setLoading(true);
    attendanceService
      .insertHolidayList({ list: props.data })
      .then(() => {
                ...
                return (
                  <Fragment key={index}>
                    {errorCode === 404 ? (
                      ...
                      <li>
                        Holidaye <strong>{item.holidayDate}</strong> has already
                        present for the year <strong>{item.reason}</strong>.
                      </li>
                    )}
                  </Fragment>
                );
...
export default HolidayFinalSubmit;
